// @gamekit/summon — pure, genre-reusable gacha "summon" logic.
//
// NO express, NO DOM, NO server/UI deps. Everything here is a plain function over
// plain data so it can be unit-tested in isolation and reused across games without
// dragging a runtime with it. An HTTP server (which owns the per-guest session
// state) and a browser client both call INTO this module; this module never
// imports from them.
//
...
//
// This is the canonical library version. The examples/gacha-game embeds a snapshot
// of this engine for fork-portability; keep the two in sync at the logic level, but
// neither depends on the other. The self-contained `mulberry32` RNG below is kept
// deliberately local (no cross-package dep) so this engine stays standalone.
...
// Kept self-contained (no cross-package dep) so this engine stays standalone.
// ---------------------------------------------------------------------------
...
/**
 * The per-guest summon state the reducers own. An HTTP server stores one of
