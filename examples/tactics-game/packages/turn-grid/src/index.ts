// @tactics/turn-grid — pure, genre-reusable turn-based grid logic.
//
// NO Phaser, NO Colyseus, NO DOM. Everything here is a plain function over plain
// data so it can be unit-tested in Node and later graduate to a top-level
// packages/* without dragging a runtime with it. The Colyseus server (which owns
// @colyseus/schema state) and the Phaser client both call INTO this module; this
...
/** Minimal per-unit shape the rotation + validation helpers reason about. The
